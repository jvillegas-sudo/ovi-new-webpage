  ...
  return (
    <Section className="relative overflow-hidden bg-[#16235d] py-24 md:py-32">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-[-18%] top-0 h-[28rem] w-[52rem] bg-[#24357f] [clip-path:polygon(0_0,100%_0,58%_100%,0_100%)] opacity-95" />
          ...
          {services.map((service) => (
            <article key={service.title} className="mx-auto flex w-full max-w-sm flex-col items-center text-center">
              <div className="relative mb-10 flex h-80 w-full items-center justify-center">
                <div
                  aria-hidden="true"
                  className={`absolute top-5 h-64 w-64 bg-[rgba(53,73,170,0.32)] blur-sm ${hexagonClassName}`}
                />
                <div
                  aria-hidden="true"
                  className={`absolute top-11 h-56 w-56 bg-[linear-gradient(180deg,rgba(126,196,255,0.72)_0%,rgba(31,51,130,0.58)_32%,rgba(31,44,118,0.88)_100%)] ${hexagonClassName}`}
                />
                <div
                  aria-hidden="true"
                  className={`absolute top-16 h-52 w-52 bg-[rgba(24,35,99,0.9)] ${hexagonClassName}`}
                />
                <div
                  aria-hidden="true"
                  className={`absolute top-8 h-14 w-14 rounded-full bg-[radial-gradient(circle,rgba(170,223,255,0.95)_0%,rgba(108,170,255,0.65)_42%,transparent_75%)] blur-md`}
                />
                <div
                  aria-hidden="true"
              ...
              <Button
                type="button"
                className="mt-10 h-16 min-w-52 rounded-full border-0 bg-[linear-gradient(90deg,#4bb26d_0%,#1b2f7d_100%)] px-10 text-xl font-extrabold uppercase tracking-wide text-white shadow-[0_20px_40px_rgba(9,21,76,0.42)] hover:brightness-105"
              >
                Ver más
              </Button>
            </article>
